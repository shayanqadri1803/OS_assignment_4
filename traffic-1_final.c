...
static pthread_cond_t condrepair = PTHREAD_COND_INITIALIZER;
static pthread_cond_t condentry = PTHREAD_COND_INITIALIZER;
static pthread_cond_t condexit = PTHREAD_COND_INITIALIZER;
int s;
...
repair_street() {
	...
	sleep(5);
	printf("The street is ready to use now. \n");
}
...
void *street_thread(void *junk) {
	...
	while (1) {
		...
		if ((cars_since_repair == USAGE_LIMIT) && (cars_on_street == 0))
		{
			repair_street();
			cars_since_repair = 0;
			s = pthread_cond_broadcast(&condrepair);
			
		}
		else if(cars_since_repair + cars_on_street == USAGE_LIMIT){
			s = pthread_cond_wait(&condexit, &mutex);
		}
		else if(cars_on_street < ALLOWED_CARS){
			s = pthread_cond_wait(&condentry, &mutex);
		}
		else{
			s = pthread_cond_wait(&condexit, &mutex);
		}
...
incoming_enter() {
	 ...
	 */
	s = pthread_mutex_lock (&mutex);
	if ((cars_on_street < ALLOWED_CARS) && (outgoing_onstreet == 0) && (cars_since_repair+cars_on_street < USAGE_LIMIT))
	{
	cars_on_street++;
	incoming_onstreet++;
	// cars_since_repair++;
	s = pthread_cond_broadcast(&condentry);
	}
	else{
		while (!(((cars_since_repair + cars_on_street) != USAGE_LIMIT ) && (cars_on_street < ALLOWED_CARS) && (outgoing_onstreet == 0))){
		if(cars_on_street+cars_since_repair == USAGE_LIMIT){
			s=pthread_cond_wait(&condrepair, &mutex);
		}
		else{
			s = pthread_cond_wait(&condexit, &mutex);
		}
...
outgoing_enter() {
	 ...
	 */
	s = pthread_mutex_lock (&mutex);
	if ((cars_on_street < ALLOWED_CARS) && (incoming_onstreet == 0) && (cars_since_repair+cars_on_street < USAGE_LIMIT))
	{
	cars_on_street++;
	outgoing_onstreet++;
	// cars_since_repair++;
	...
	else{while(!(((cars_since_repair + cars_on_street) != 7) && (cars_on_street < 3) && (incoming_onstreet == 0))){
...
incoming_leave() {
	...
	incoming_onstreet--;
	cars_since_repair++;
	s = pthread_cond_broadcast(&condexit);
	s = pthread_mutex_unlock(&mutex);
